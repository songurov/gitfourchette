...
# Status icons generated by generateIcons(): delta status -> glyph on the tile.
STATUS_ICON_GLYPHS = {
    "A": "A",  # added
    "D": "D",  # deleted
    "M": "M",  # modified
...
def statusIconSvg(glyph: str) -> str:
    # Monochrome, like every other semantic icon: the glyph tells the statuses
    # apart, not a hue. test_iconbank.py rejects hardcoded colors in these.
    return "\n".join([
        "<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 16 16'>",
        "<rect rx='2' x='1' y='1' width='14' height='14' stroke='gray' stroke-width='1.4' fill='none'/>",
        f"<text x='8' y='12' font-weight='bold' font-size='11' font-family='sans-serif' text-anchor='middle' fill='gray'>{glyph}</text>",
        "</svg>",
        "",
    ])


def writeStatusIcon(status: str, glyph: str):
    svgPath = Path(ASSETS_DIR) / f"icons/status_{status.lower()}.svg"
    writeIfDifferent(svgPath, statusIconSvg(glyph))
...
def generateIcons():
    ...
    for status, glyph in STATUS_ICON_GLYPHS.items():
        writeStatusIcon(status, glyph)
